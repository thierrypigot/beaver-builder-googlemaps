=== Beaver Builder Google Maps Module ===
Contributors: thierrypigot
Donate link: https://www.thierry-pigot.fr/reglement/
Tags: builder, page builder, visual composer, google maps, googlemaps, widget, module, marker
Requires at least: 3.6
Tested up to: 4.3
Stable tag: trunk
License: GPL2+
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Add a new module to create a Google Maps in Beaver Builder.


== Description ==

Map options
1. Locate marker by Lattitude and Longitude
2. Zoom level
3. Map height
4. Custom marker
5. Custom text (wysiwyg) inside marker popup


== Installation ==

1. /!\ Need Beaver Builder to work.
2. Install and activate Beaver Builder Google Maps Module either via the WordPress plugin directory, or by uploading the files to your server at wp-content/plugins.
3. Beaver Builder Google Maps Module will now be available to use for any of the post types you chose.

== Screenshots ==

1. Screenshot of settings module 1/2
2. Screenshot of settings module 2/2
3. Result on front page

== Changelog ==

= 1.2 =
* Add multi markers
* Add some filters

= 1.1 =
* First stable version